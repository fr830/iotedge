...
fn run() -> Result<(), Error> {
    ...
    match matches.subcommand() {
        ("list", Some(_args)) => {
            tokio_runtime.block_on_all(List::new(runtime, io::stdout()).execute())
        }
        ("restart", Some(args)) => tokio_runtime.block_on_all(
            Restart::new(
        ...
        ("logs", Some(args)) => {
            ...
            tokio_runtime.block_on_all(Logs::new(id, options, runtime).execute())
        }
        ("version", Some(_args)) => tokio_runtime.block_on_all(Version::new().execute()),
        (command, _) => tokio_runtime.block_on_all(Unknown::new(command.to_string()).execute()),
    }
}
