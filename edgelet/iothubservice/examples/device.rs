    ...
    T::Error: Into<HttpError>,
{
    let response = tokio_runtime
        .block_on_all(device_client.list_modules())
        .unwrap();
...
{
    let response = tokio_runtime
        .block_on_all(device_client.get_module_by_id(module_id))
        .unwrap();
    println!("{}", serde_json::to_string_pretty(&response).unwrap());
}
...
{
    let response = tokio_runtime
        .block_on_all(device_client.create_module(module_id, None, None))
        .unwrap();
    println!("{}", serde_json::to_string_pretty(&response).unwrap());
}
...
{
    tokio_runtime
        .block_on_all(device_client.delete_module(module_id))
        .unwrap();
