...
use hyper::{self, Body, Error as HyperError, Method, Request, Response, Uri};
        ...
        self.host_name
            ...
            .and_then(|url| {
                ...
                *req.method_mut() = method;
                ...
                let url = url.as_str().parse::<Uri>().expect("Unexpected Url to Uri conversion failure");
                *req.uri_mut() = url;

                // add user agent header
                if let Some(ref user_agent) = self.user_agent {
                    req.headers_mut().append(http::header::USER_AGENT, user_agent.parse().unwrap());
                }
                ...
                if add_if_match {
                    req.headers_mut().append(http::header::IF_MATCH, http::header::HeaderValue::from_static("*"));
                }
                ...
                if let Some(body) = body {
                    ...
                    req.headers_mut().typed_insert(&ContentLength(serialized.len() as u64));

                    *req.body_mut() = Body::from(serialized);
                }
